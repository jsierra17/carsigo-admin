...
export default function PrecioDinamicoPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20 font-sans">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-white tracking-tighter">Precio Dinámico</h1>
          </div>
          <p className="text-slate-400 font-medium">Reglas de aumento de tarifa por hora, día, fecha especial o zona.</p>
        </div>
        {!showForm && (
          <button onClick={() => { setShowForm(true); setEditingId(null); setForm(defaultForm) }}
            className="flex items-center gap-2 px-6 py-3 bg-[#00E5FF] hover:bg-[#00D0E8] text-[#131313] rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-95 focus-visible:ring-2 focus-visible:ring-[#00E5FF]/60">
      ...
      {showForm && (
        <div className="bg-[#141416] border border-white/5 p-8 rounded-3xl shadow-sm">
          ...
          <form onSubmit={handleSave} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                  placeholder="Ej: Hora Pico Mañana" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Tipo de Regla</label>
              ...
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Multiplicador (×)</label>
              ...
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Recargo fijo ($)</label>
                <input type="number" min={0} value={form.flat_surcharge} onChange={e => setForm({ ...form, flat_surcharge: Number(e.target.value) })}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Prioridad</label>
                <input type="number" min={0} max={999} value={form.priority} onChange={e => setForm({ ...form, priority: Number(e.target.value) })}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
              </div>
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Vehículo (opcional)</label>
            ...
            {(form.rule_type === 'time_range' || form.rule_type === 'demand_zone') && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Hora Inicio</label>
                  <input type="time" value={form.start_time} onChange={e => setForm({ ...form, start_time: e.target.value })}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Hora Fin</label>
            ...
            {(form.rule_type === 'time_range' || form.rule_type === 'day_of_week' || form.rule_type === 'demand_zone') && (
              <div>
                <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-3">Días de la Semana</label>
                  ...
                  {dayLabels.map((label, i) => {
                    ...
                    return (
                      <button key={dayNum} type="button" onClick={() => toggleDay(dayNum)}
                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border focus-visible:ring-2 focus-visible:ring-[#00E5FF]/60 ${selected ? 'bg-[#00E5FF] text-[#131313] border-[#00E5FF] shadow-sm' : 'bg-white/5 text-slate-400 border-white/10 hover:border-[#00E5FF]/30 hover:text-[#00E5FF]'}`}>
                        {label}
                      </button>
                    )
                  })}
            ...
            {form.rule_type === 'specific_date' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Fecha</label>
                ...
                <div className="flex items-end pb-3">
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Repetir cada año</span>
                  </label>
                </div>
              </div>
            )}
            ...
            {form.rule_type === 'date_range' && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Desde</label>
                  <input type="date" value={form.date_from} onChange={e => setForm({ ...form, date_from: e.target.value })}
                    className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
                </div>
                <div>
                  <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Hasta</label>
            ...
            <div>
              <label className="text-xs text-slate-500 font-black uppercase tracking-widest block mb-2">Descripción (opcional)</label>
              <input type="text" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                placeholder="Ej: Aumento por hora pico de la mañana" className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm font-bold text-white placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-[#00E5FF]/40 focus:border-[#00E5FF]/40" />
            </div>
      ...
      <div className="bg-[#141416] border border-white/5 rounded-3xl shadow-sm overflow-hidden">
        ...
        {isLoading ? (
          ...
          <div className="p-20 text-center">
            ...
            <p className="text-slate-400 text-sm mt-2">Agrega reglas para aumentar tarifas en horas pico, fines de semana o fechas especiales.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white/2">
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Nombre</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Tipo</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Detalle</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Mult.</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Recargo</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Prioridad</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Estado</th>
                  <th className="px-6 py-5 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-right">Acción</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {visibleRules.map(rule => (
                  <tr key={rule.id} className="hover:bg-[#00E5FF]/5 transition-colors">
                    ...
                    <td className="px-6 py-5">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-purple-500/10 text-purple-400 rounded-full text-[10px] font-black uppercase tracking-wider border border-purple-500/20">
                        {getTypeIcon(rule.rule_type)} {ruleTypeLabels[rule.rule_type]}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-xs font-medium text-slate-400 max-w-[200px] truncate">{getSummary(rule)}</td>
                    <td className="px-6 py-5 font-black text-orange-400">×{rule.multiplier}</td>
                    <td className="px-6 py-5 font-bold text-slate-300">{rule.flat_surcharge > 0 ? `$${rule.flat_surcharge.toLocaleString('es-CO')}` : '—'}</td>
                    <td className="px-6 py-5">
                      <span className="font-mono text-xs font-bold text-slate-400">{rule.priority}</span>
                    </td>
                    <td className="px-6 py-5">
                      <button onClick={() => handleToggle(rule)}
                        className={`px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border focus-visible:ring-2 focus-visible:ring-[#00E5FF]/60 ${rule.is_active ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => handleEdit(rule)}
                          className="p-2 text-[#00E5FF] bg-[#00E5FF]/10 hover:bg-[#00E5FF]/20 rounded-xl transition-all focus-visible:ring-2 focus-visible:ring-[#00E5FF]/60" title="Editar">
                          <Pencil size={14} />
                        </button>
                        <button onClick={() => handleDelete(rule.id, rule.name)}
                          className="p-2 text-red-400 bg-red-500/10 hover:bg-red-500/20 rounded-xl transition-all focus-visible:ring-2 focus-visible:ring-[#00E5FF]/60" title="Eliminar">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
